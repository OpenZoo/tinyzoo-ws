...
void ElementCentipedeHeadTick(uint8_t stat_id) {
	...
	if (head_x == ZOO_STAT(0).x && (ZRAND10() < stat->p1)) {
		stat->step_y = signum8(ZOO_STAT(0).y - head_y);
		stat->step_x = 0;
	} else if (head_y == ZOO_STAT(0).y && (ZRAND10() < stat->p1)) {
		stat->step_x = signum8(ZOO_STAT(0).x - head_x);
		stat->step_y = 0;
	} else if (((ZRAND10() << 2) < stat->p2) || (stat->step_x == 0 && stat->step_y == 0)) {
